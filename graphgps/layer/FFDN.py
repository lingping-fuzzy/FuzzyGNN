...
from graphgps.layer.FuzzyLayer import FuzzyLayer
...
class FusedFuzzyDeepNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        device1 = fuzz_input.get_device()
        if device1 == -1:
            device1 = 'cpu'
        fuzz_output = torch.zeros(input.size(), dtype=torch.float, device= device1)
