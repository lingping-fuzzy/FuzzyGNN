...
class AntecedentLayer(torch.nn.Module):
    ...
    def __init__(self, varlist):
        ...
        self.register_buffer(name='mf_indices', tensor=torch.tensor(list(mf_indices)))
...
class PolyConsequentLayer(ConsequentLayer):
    '''
        A linear layer to represent the TSK consequents.
        Not hybrid learning, so coefficients are backprop-learnable parameters.
    '''
    def __init__(self, *params):
        super(PolyConsequentLayer, self).__init__(*params)
        self.register_parameter('coefficients',
                                torch.nn.Parameter(self._coeff))
        hotidx = np.zeros(self.d_in)
        ing_idx = np.random.choice(np.arange(self.d_in), int(self.d_in*0.5), replace=False)
        hotidx[ing_idx] = 1
        self.ploy_mask = Parameter(torch.from_numpy(hotidx).type(torch.bool), requires_grad=False)
    ...
    def forward(self, x):
        ...
        x_2 = torch.ones_like(x, device=x.device)
        x_3 = torch.where(self.ploy_mask, x, x_2)
        x = torch.mul(x_3, x)
        x_plus = torch.cat([x, torch.ones(x.shape[0], 1, device=x.device)], dim=1)
        # Need to switch dimansion for the multipy, then switch back:
        y_pred = torch.matmul(self.coeff, x_plus.t())
...
class AnfisNet(torch.nn.Module):
    ...
    '''
    def __init__(self, description, invardefs, outvarnames, hybrid=True, used_feature=10, division = 'random'):
        ...
        n_used_feature = len(varnames)

        self.division = division
        if self.division == 'random':
            onehot = np.eye(used_feature)
            using_idx = np.random.choice(np.arange(used_feature), n_used_feature, replace=False)
        else:
            onehot = np.eye(used_feature)
            using_idx = np.random.choice(np.arange(used_feature), n_used_feature, replace=False)
        ...
        if self.hybrid:
            cl = PolyConsequentLayer(self.num_in, self.num_rules, self.num_out)
        else:
    ...
    def forward(self, x_in):
        ...
        '''
        # first to map to lower-dimension
        x = torch.mm(x_in, self.feature_mask)
